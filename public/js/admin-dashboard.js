...
if (user) {
  ...
  set('sidebarDivision',     user.division || 'Division Office');
  set('topbarAdminName',     user.name);
  set('adminWelcome',        'Welcome, ' + user.name);
  set('adminDivisionLabel',  user.division || 'Division Office');
  set('settingsAdminName',   user.name);
  set('settingsDivision',    user.division || 'Division Office');
}
...
function switchPage(pageId) {
  ...
  const titles = { dashboard:'Dashboard', submissions:'All Submissions', schools:'Registered Schools',
...
async function loadAdminDashboard() {
  try {
    ...
    tbody.innerHTML = pending.length ? pending.map(s =>
      '<tr><td><strong>' + API.escapeHtml(s.ref) + '</strong></td>' +
      '<td>' + API.escapeHtml(s.school_name || '') + '</td>' +
      '<td>' + API.escapeHtml(s.doc_type) + '</td>' +
      '<td>' + new Date(s.submitted_at).toLocaleDateString('en-PH') + '</td>' +
      '<td><button class="action-btn review" onclick="openReview(\'' + API.escapeHtml(s.ref) + '\')"><i class="fas fa-eye"></i> Review</button></td></tr>'
...
function renderActivity(subs) {
  ...
  list.innerHTML = subs.map(s => {
    ...
    const text  = s.status === 'approved'
      ? '<strong>' + API.escapeHtml(s.ref) + '</strong> approved \u2013 ' + API.escapeHtml(s.doc_type) + ' from ' + API.escapeHtml(s.school_name || '')
      : s.status === 'returned'
      ? '<strong>' + API.escapeHtml(s.ref) + '</strong> returned to ' + API.escapeHtml(s.school_name || '') + ' \u2013 ' + API.escapeHtml(s.doc_type)
      : '<strong>' + API.escapeHtml(s.ref) + '</strong> submitted by ' + API.escapeHtml(s.school_name || '') + ' \u2013 ' + API.escapeHtml(s.doc_type);
    return '<div class="activity-item"><div class="activity-dot ' + color + '"></div>' +
      '<div class="activity-text">' + text + '</div>' +
      '<div class="activity-time">' + API.timeAgo(s.submitted_at) + '</div></div>';
  ...
  try {
    ...
    tbody.innerHTML = subs.length ? subs.map(s => {
      ...
      return '<tr>' +
        ...
        '<td><button class="action-btn review" onclick="openReview(\'' + API.escapeHtml(s.ref) + '\')"><i class="fas fa-eye"></i> Review</button></td>' +
        '</tr>';
    }).join('') : '<tr><td colspan="11" style="text-align:center;color:var(--text-muted);padding:32px">No submissions found.</td></tr>';

    tbody.querySelectorAll('.row-check').forEach(cb => {
      cb.addEventListener('change', () => {
        if (cb.checked) { if (!selectedRefs.includes(cb.dataset.ref)) selectedRefs.push(cb.dataset.ref); }
        else selectedRefs = selectedRefs.filter(r => r !== cb.dataset.ref);
        updateBulkBar();
      });
    });
...
document.getElementById('selectAll').addEventListener('change', function() {
  document.querySelectorAll('.row-check').forEach(cb => {
    ...
    if (this.checked) { if (!selectedRefs.includes(cb.dataset.ref)) selectedRefs.push(cb.dataset.ref); }
    else selectedRefs = selectedRefs.filter(r => r !== cb.dataset.ref);
  });
  updateBulkBar();
});
...
function updateBulkBar() {
  ...
  if (selectedRefs.length > 0) { bar.removeAttribute('hidden'); document.getElementById('bulkCount').textContent = selectedRefs.length + ' selected'; }
...
async function openReview(ref) {
  ...
  try {
    ...
    document.getElementById('reviewDetails').innerHTML =
      ...
      (s.files && s.files.length ? '<br/><strong>Download Files:</strong> ' +
        s.files.map(f => '<a href="/api/submissions/' + s.ref + '/files/' + f.id + '?token=' + localStorage.getItem('smme_token') + '" target="_blank" class="action-btn view" style="margin:2px"><i class="fas fa-download"></i> ' + API.escapeHtml(f.original_name) + '</a>').join(' ') : '');
  ...
  try {
    ...
    document.getElementById('schoolsGrid').innerHTML = filtered.map(s =>
      ...
      '<div class="school-card-header">' +
        ...
        '<div><div class="school-card-name">' + API.escapeHtml(s.name) + '</div><div class="school-card-id">' + s.school_code + '</div></div>' +
      '</div>' +
...
document.getElementById('schoolSearch').addEventListener('input', loadSchools);
...
document.getElementById('staffSearch').addEventListener('input', loadStaffPage);
...
async function loadReports() {
  try {
    ...
    const now = new Date();
    const thisMonth = subs.filter(s => { const d = new Date(s.submitted_at); return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear(); });
    document.getElementById('rptThisMonth').textContent = thisMonth.length;

    const byType = {}; subs.forEach(s => { byType[s.doc_type] = (byType[s.doc_type]||0) + 1; });
    ...
    document.getElementById('reportBars').innerHTML = typeEntries.map(([label, count]) =>
      '<div class="report-bar-item"><div class="report-bar-label">' + API.escapeHtml(label) + '</div>' +
      '<div class="report-bar-track"><div class="report-bar-fill" style="width:' + Math.round(count/maxType*100) + '%"></div></div>' +
      '<div class="report-bar-count">' + count + '</div></div>'
    ).join('') || '<p style="color:var(--text-muted);padding:16px">No data yet.</p>';

    const bySchool = {}; subs.forEach(s => { bySchool[s.school_name||'Unknown'] = (bySchool[s.school_name||'Unknown']||0) + 1; });
    ...
    document.getElementById('reportBySchool').innerHTML = schoolEntries.map(([label, count]) =>
      '<div class="report-bar-item"><div class="report-bar-label" style="width:160px">' + API.escapeHtml(label) + '</div>' +
      '<div class="report-bar-track"><div class="report-bar-fill" style="width:' + Math.round(count/maxSchool*100) + '%;background:var(--success)"></div></div>' +
      '<div class="report-bar-count">' + count + '</div></div>'
    ).join('') || '<p style="color:var(--text-muted);padding:16px">No data yet.</p>';
...
async function loadAuditLog() {
  ...
  try {
    ...
    document.getElementById('auditLogList').innerHTML = log.length ? log.map(e =>
      ...
      '<div class="audit-text"><strong>' + API.escapeHtml(e.ref||'\u2014') + '</strong>' +
      ...
      '</div>' +
      '<span class="audit-time">' + API.timeAgo(e.created_at) + '</span></div>'
...
async function loadDeadlineMgmt() {
  try {
    ...
    list.innerHTML = deadlines.length ? deadlines.map(d => {
      ...
      return '<div class="deadline-mgmt-row">' +
        ...
        '</div>' +
        '<div style="flex:1"><strong style="font-size:.9rem">' + API.escapeHtml(d.doc_type) + '</strong>' +
          '<span style="font-size:.78rem;color:var(--text-muted);display:block">SY ' + API.escapeHtml(d.school_year) + ' | ' + API.escapeHtml(d.level === 'all' ? 'All Levels' : API.levelLabel(d.level)) + ' | Due: ' + API.escapeHtml(d.deadline) + '</span>' +
        '</div>' +
...
async function loadNotices() {
  try {
    ...
    list.innerHTML = notices.map(n => {
      ...
      return '<div class="notice-item notice-' + n.type + '">' +
        '<i class="fas ' + ic + '"></i>' +
        '<div style="flex:1"><strong>' + API.escapeHtml(n.title) + '</strong><p>' + API.escapeHtml(n.message) + '</p>' +
        '<span class="notice-date">' + new Date(n.created_at).toLocaleDateString('en-PH') + '</span></div>' +
