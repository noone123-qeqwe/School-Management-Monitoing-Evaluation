  ...
  try {
    ...
    document.getElementById('reviewDetails').innerHTML =
      ...
      (s.files && s.files.length ? '<br/><strong>Download Files:</strong> ' +
        s.files.map(f =>
          '<button class="action-btn view" style="margin:2px" ' +
          'onclick="downloadFile(\'' + API.escapeHtml(s.ref) + '\',' + f.id + ',\'' + API.escapeHtml(f.original_name) + '\')">' +
          '<i class="fas fa-download"></i> ' + API.escapeHtml(f.original_name) + '</button>'
        ).join(' ') : '');
