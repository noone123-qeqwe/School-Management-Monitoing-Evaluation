...
function switchPage(pageId) {
  ...
  const titles = { dashboard:'Dashboard', submit:'Submit Documents', submissions:'School Submissions',
...
function renderTableEmpty(tbody, colspan, message) {
  if (!tbody) return;
  tbody.innerHTML = `<tr><td colspan="${colspan}" style="text-align:center;color:var(--text-muted);padding:32px">${API.escapeHtml(message)}</td></tr>`;
}

function renderBlockError(container, message, retryJs) {
  if (!container) return;
  container.innerHTML = `
    <div style="text-align:center;padding:16px">
      <p style="color:var(--danger);margin-bottom:10px">${API.escapeHtml(message)}</p>
...
async function loadDivisionNotices() {
  ...
  try {
    ...
    renderBlockError(list, 'Failed to load notices.', 'loadDivisionNotices()');
  }
}
...
async function loadTasksSummary() {
  ...
  try {
    ...
    renderBlockError(list, 'Unable to load tasks right now.', 'loadTasksSummary()');
    progressEl.innerHTML = '';
  }
}
...
function renderRecentTable(subs) {
  ...
  if (!subs.length) renderTableEmpty(tbody, 8, 'No submissions yet. Create your first submission.');
}
...
async function loadSubmissions(mode) {
  ...
  try {
    ...
    const filtered = mode === 'mine' ? subs.filter(s => s.staff_id === user.id) : subs;
    const tbodyId  = mode === 'mine' ? 'mineSubmissionsBody' : 'allSubmissionsBody';
    const cols     = mode === 'mine' ? 8 : 10;
    const tbody    = document.getElementById(tbodyId);
    if (!tbody) return;
    if (!filtered.length) {
      renderTableEmpty(tbody, cols, mode === 'mine' ? 'No personal submissions match your filters.' : 'No school submissions match your filters.');
      return;
    }
    ...
    const tbody = document.getElementById(mode === 'mine' ? 'mineSubmissionsBody' : 'allSubmissionsBody');
    const cols = mode === 'mine' ? 8 : 10;
    renderTableEmpty(tbody, cols, 'Unable to load submissions right now.');
  }
}
...
function prefillTrack(ref) {
  ...
  setTimeout(() => document.getElementById('dTrackBtn').click(), 100);
}
