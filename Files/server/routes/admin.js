...
router.get('/notices/:id/stats', requireAdmin, async (req, res) => {
  try {
    ...
    res.json({ audience, viewedSchools: viewed, viewRate, trend: trendResult.rows });
  } catch (err) {
    res.status(500).json({ error: 'Server error.' });
  }
});
...
router.get('/notices-analytics', requireAdmin, async (req, res) => {
  try {
    const [noticesRes, viewsRes, audienceRes, trendRes] = await Promise.all([
      pool.query('SELECT id FROM notices'),
      pool.query('SELECT COUNT(*)::int AS views FROM notice_views'),
      pool.query('SELECT COUNT(*)::int AS audience FROM schools'),
      pool.query(
        `SELECT DATE(viewed_at) AS day, COUNT(*)::int AS views
         FROM notice_views
    ...
    const totalNotices = noticesRes.rows.length;
    const totalViews = viewsRes.rows[0]?.views || 0;
    const audience = audienceRes.rows[0]?.audience || 0;

    const viewedSchoolsRes = await pool.query(
      'SELECT COUNT(DISTINCT school_id)::int AS viewed_schools FROM notice_views'
    );
    const viewedSchools = viewedSchoolsRes.rows[0]?.viewed_schools || 0;
